...
"""
Implementation of a simple LRU(Least Recently Used) caching algorithm.
...
class LRUCache(BaseCaching):
    """
    Implements a simple `LRU` caching algorithm.
    ...
    Contains 4 methods:
        ...
        * `get_LRU_item` (private) - Returns the key of the least
        used cache item
    """

    def __init__(self):
        """
        Initialize a LRUCacing object with the necessary attributes (see below)
        """
    ...
    def put(self, key, item):
        ...
        accordingly. If cache is full, discard the least recently used(LRU)
        cache item. The LRU is determined by reading the `age_bit_table`. Once
        ...
        if len(self.cache_data) >= self.MAX_ITEMS:
            LRU_item_key = self.__get_LRU_item()
            print(f'DISCARD: {LRU_item_key}')
            del self.cache_data[LRU_item_key]
            del self.__age_bit_table[LRU_item_key]
    ...
    def __get_LRU_item(self):
        ...
        smallest_agebit = sorted(items)[0]
